...
from Preprocessing.config.graph_keys import GRAPH_KEYS, TOOL
from Preprocessing.utils import misc
from Preprocessing.learning.learning_utils import *
        ...
        for batch_idx, (graph, K, graph_sizes, node_ids) in enumerate(train_loader):
            ...
            if batch_idx % learning_routine.print_each == 0:
                ...
                print(
                    ...
                    f'Loss: {loss:.6f}  Time: {time_elapsed:.2f}  LossT:{lossT:.2f}')
                lossT = 0
                # tensorboard logging
...
def train_supervised(model,
    ...
    for epoch in range(learning_routine.num_epochs):
        ...
        for batch_idx, (graph, indxs, node_features) in enumerate(train_loader):
            # Get data on the devices
            # graph = send_graph_to_device(graph, device)
            graph = graph.to(device)
            # Do the computations for the forward pass
...
if __name__ == '__main__':
    pass
    from RGCN.learning import models
