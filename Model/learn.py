...
class LearningRoutine:
    ...
    def __init__(self,
        ...
        self.test18_loader = test18_loader
...
def compute_outputs(model, validation_loader):
    ...
    y_pred = Binarizer(threshold=0.5).fit_transform(predicted.reshape(-1,1))
    mcc = matthews_corrcoef(true, y_pred)

    precision = precision_score(true, y_pred)
    recall = recall_score(true, y_pred)

    print("Vaild MCC指标为：", mcc)
    print("Vaild Precision:", precision)
    print("Vaild Recall:", recall)
...
def train_supervised(model,
    ...
    for epoch in range(learning_routine.num_epochs):
        ...
        for batch_idx, (graph, indxs , features ,seqs, seqlens, len_graphs, chain_idx) in enumerate(train_loader):
            # Get data on the devices
            # graph = send_graph_to_device(graph, device)
            ...
            if model.return_loss:
                out, constraint_loss = model(graph ,features)
            else:
                out = model(graph ,features, indxs ,seqs, seqlens, len_graphs, chain_idx )
                constraint_loss = 0
            labels = graph.ndata['target']
           ...
           # mse_loss = torch.nn.MSELoss()(out, newlabel)
            logicloss = 0
            pos_w = 1#7.31
            neg_w = 1#1.37
            #weights = torch.tensor([[1.0, 2.0]])
            for idx in range(len(newlabels)):
                if labels[idx] > 0 :
                   logicloss += pos_w * torch.nn.BCELoss()(out[idx],newlabels[idx])
                else:
                    logicloss += neg_w * torch.nn.BCELoss()(out[idx],newlabels[idx])
            #logicloss +=  torch.nn.BCELoss(weight=weights)(out,labels)
            logicloss /= len(newlabels)
            loss = constraint_loss + logicloss
        ...
        if learning_routine.validation_loader is None:
            ...
            early_stop = learning_routine.early_stopping_routine(validation_loss=validation_loss, epoch=epoch,
                                                                 model=model, optimizer=optimizer)
            validation_losstest3 = evaluate_model_supervised(model, validation_loader=learning_routine.test3_loader)
            validation_losstest18 = evaluate_model_supervised(model, validation_loader=learning_routine.test18_loader)
            print(f"Test18AUC: {validation_losstest18}")
            print(f"Test3AUC: {validation_losstest3}")
            #if validation_loss > 0.77 :
            if validation_losstest3 > 0.783 and validation_losstest18 > 0.776:
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict()
                }, f'/mnt/sdd/user/wjk/MyRNAPredict/crosstest18_0.3/Same7_1024_42_10_{epoch}_{validation_losstest3}_{validation_losstest18}.pth')
        if early_stop:
            pass
            #break
    return learning_routine.best_loss
